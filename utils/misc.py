...
CHO_BEATMAP_REGEX = re.compile(
    r"^https://osu\.ppy\.sh/beatmapsets/(?P<sid>\d{1,10})#/?(?P<mode>:?osu|taiko|fruits|mania)?/(?P<bid>\d{1,10})/?$"
)
SAKURU_BEATMAP_REGEX = re.compile(r"^https://sakuru\.pw/b/(?P<bid>\d{1,10})")

_win_cond_str = ("Score", "Accuracy", "Combo", "ScoreV2")

_team_types_str = ("Head To Head", "TAG Coop", "TeamVS", "TAG TeamVS")
...
def _get_required_score_for_level(level: int) -> float:
    if level <= 100:
        if level > 1:
            return round(
                5000 / 3 * (4 * pow(level, 3) - 3 * pow(level, 2) - level)
                + round(1.25 * pow(1.8, level - 60))
            )
        else:
            return 1
    else:
...
def sakuro_error(error: str, title: str, color: any) -> Embed:
    ret = Embed(title=title, description=error, color=color, timestamp=datetime.now())
...
def parse_history(
    action: str, old_value: Optional[str], new_value: str, users: dict[int, str]
) -> str:
