...
from utils.misc import get_completion
from utils.misc import get_level
from utils.misc import get_level_percent
from utils.misc import parse_history
from utils.misc import sakuro_error
from utils.wrappers import check_args
from utils.wrappers import link_required
...
class OsuCog(commands.Cog, name="Osu"):
    ...
    async def osu(
        ...
        isDonor = (
            f'\n\n**▸ {player["name"]} is a Supporter:** ♥️'
            if scope["info"]["priv"] & 48
            else ""
        )

        embed = Embed(
            description=f"**▸ Official Rank:** #{stats['rank']} "
        ...
        embed.set_author(
            name=f"{mode.upper()}!{mods.upper()} Profile for {player['name']}",
    ...
    async def recent(
        ...
        score = (
            await UserHelper.getUserScores(
                player["safe_name"], convert_mode_int(mode), mods, 1, "recent"
            )
        )[0]
        ...
        choke_str = (
            f'*({calc["pp"]:.2f}PP for {score["acc"]:.2f}% FC)*'
            if score["grade"] not in ("S", "SS", "X", "SH")
            else ""
        )
        completion_str = (
        ...
        embed = Embed(
            description=f"▸ {convert_grade_emoji(score['grade'])} ▸ **{score['pp']:.2f}PP** {choke_str}"
            ...
            timestamp=datetime.fromisoformat(score["play_time"]),
            colour=ctx.author.color,
        )

        mods_str = f' +{Mods(score["mods"])!r} ' if score["mods"] != 0 else " "
        embed.set_author(
        ...
        embed.set_footer(
            text=f"Played on Sakuru.pw server. | Map By {calc['map_creator']}"
        )
    ...
    async def topscores(
        ...
        scores = await UserHelper.getUserScores(
            player["safe_name"], convert_mode_int(mode), mods, 100, "best"
        )
        ...
        for idx, score in enumerate(scores):
            ...
            description += (
                f"""** {idx + 1}. [{calc['map_fullname']}](https://skuru.pw/direct?id={score['beatmap']['id']})"""
    ...
    async def current(
        ...
        for idx, score in enumerate(scores):
            ...
            description += (
                f"""** {idx + 1}. {mods_str}** [{calc['stars']:.2f}★]\n"""
                f"▸ {convert_grade_emoji(score['grade'])} ▸ **{score['pp']:.2f}PP** {choke_str}"
                f"▸ {score['acc']:.2f}%\n▸ {score['score']} ▸ x{score['max_combo']}/{score['beatmap']['max_combo']} "
                f"▸ [{score['n300']}/{score['n100']}/{score['n50']}/{score['nmiss']}]\n"
                f"▸ Score Set <t:{datetime.fromisoformat(score['play_time']).timestamp().__int__()}:R>\n"
        ...
        embed.set_author(
            name=f"Top {len(scores)} Plays for {player['name']} on {map_fullname}",
            url=f"https://sakuru.pw/u/{player['id']}",
            icon_url=f"https://sakuru.pw/static/flags/{player['country'].upper()}.png",
        )
        embed.set_footer(
            text="On Sakuru.pw server.", icon_url="https://sakuru.pw/static/ingame.png"
        )
        embed.set_thumbnail(url=f"https://a.sakuru.pw/{player['id']}")
