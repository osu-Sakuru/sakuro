    ...
    async def replay(self, ctx: ContextWrap, nickname: str, mods: str, map_id: int):
        ...
        if not player:
            async with glob.http.get(
                "https://sakuru.pw/api/search", params={"q": nickname}
            ) as resp:
                if resp.status == 200:
                    ...
                    if data["length"] == 0:
                        return await ctx.send(
                            f"Nothing matched with {nickname} not found, check your spelling."
                        )

                    embed = Embed(color=ctx.author.color, timestamp=datetime.now())
                    ...
                    try:
                        ...
                        player = await UserHelper.getOsuUserByName(
                            make_safe_name(
                                data["matches"][QUEUE_EMOJIS.index(reaction.emoji)][
                                    "name"
                                ]
                            ),
                            "info",
        ...
        scores = await UserHelper.getUserScores(
            player["id"], 0, mods, 5, "best", map_id
        )
        ...
        for idx, score in enumerate(scores):
            calc = await Calculator.calculate(
                score["beatmap"]["id"], 0, score["mods"], score["acc"], None
            )
            map_fullname = calc["map_fullname"]

            description += (
                f"""** {idx + 1}. {f' +{Mods(score["mods"])!r}' if score['mods'] != 0 else ''}** [{calc['stars']:.2f}★]\n"""
                ...
                f"(https://api.sakuru.pw/get_replay?id={score['id']})\n"
            )
        ...
        embed.set_footer(
            text="Click on Score Set to download replay.",
            ...
            while valid is False:
                try:
                    ...
                    if reply.content > len(bmaps) or reply.content <= 0:
                        ...
                        if (
                            bm_status := bmaps["map"][reply.content - 1]["status"]
