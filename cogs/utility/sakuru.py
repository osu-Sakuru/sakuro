...
from utils.misc import CHO_BEATMAP_REGEX
from utils.misc import convert_status_str
from utils.misc import SAKURU_BEATMAP_REGEX
...
class SakuruCog(commands.Cog):
    ...
    async def map_reqs(self, msg: Message) -> None:
        if (
            msg.guild
            and msg.guild.id == config.SAKURU_ID
            and msg.channel.id == config.MAP_REQS
            and not msg.author.bot
        ):
            if (beatmap := CHO_BEATMAP_REGEX.search(msg.content)) is None and (
                beatmap := SAKURU_BEATMAP_REGEX.search(msg.content)
            ) is None:
                reply = await msg.reply(
                    "Map not found! You only able to post beatmap links in this channel. "
                ...
                if (set_id := beatmap.groupdict().get("sid", None)) is not None:
                    ...
                    params = {"id": beatmap.group("bid"), "set_from_map": 1}
                ...
                async with glob.http.get(
                    ...
                    if (
                        ...
                        chan_name = (
                            (chan_name[:97] + "...")
                            if len(chan_name) > 100
                            else chan_name
                        )

                        if (
                            discord.utils.find(
                                lambda x: x.name == chan_name, msg.channel.threads
                            )
                            is not None
                        ):
                        ...
                        embed.set_image(
                            url=f"https://assets.ppy.sh/beatmaps/{first['set_id']}/covers/cover.jpg"
                        )
